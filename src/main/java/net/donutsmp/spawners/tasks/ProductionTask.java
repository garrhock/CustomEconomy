...
public class ProductionTask extends BukkitRunnable {
    ...
    public void run() {
        for (SpawnerData data : manager.getSpawners().values()) {
            ...
            int amount = (int) (effectiveStack(data.getStackSize()) * multiplier);
            if (amount > 0) {
    ...
    // Diminishing returns. Straight stack size scales linearly, so a 100x stack prints 100x
    // forever; DonutSMP asymptotes instead (skeletons cap around 1505 bones/min no matter how
    // many you pile up). Small stacks stay roughly linear and a stack of 1 still yields 1.
    private double effectiveStack(int stackSize) {
        if (stackSize <= 1) return stackSize;

        var config = manager.getPlugin().getConfig();
        if (!config.getBoolean("diminishing_returns.enabled", true)) {
            return stackSize;
        }
        double softCap = config.getDouble("diminishing_returns.soft_cap", 750.0);
        if (softCap <= 0) return stackSize;

        return softCap * stackSize / (softCap + stackSize - 1);
    }
